...
void writeCompressedOutput(const char* inFile,
  ...
  while(fin.get(c)){
    unsigned char u = c;
    if(theMap.count(u) > 0){
      std::map<unsigned,BitString>::const_iterator it = theMap.find(u); 
      bfw.writeBitString(it->second);
    }else{
      std::cout << "the character  " << c << "  is not in the map" << std::endl;
      exit(EXIT_FAILURE);
    }
  }
  //dont forget to lookup 256 for the EOF marker, and write it out.
  std::map<unsigned,BitString>::const_iterator it_eof = theMap.find(256);
  bfw.writeBitString(it_eof->second);
